...
namespace io
{
    ...
    {
    public:
        static uint32_t addFunction(std::function<void()> func, const std::vector<uint32_t> &times_ms,
            const std::string &name, bool is_repeatable = false);
        static bool removeFunction(uint32_t function_id);
        static bool hasFunction(uint32_t function_id);
        static bool modifyFunctionRepeatability(uint32_t function_id, bool state);
