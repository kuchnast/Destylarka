...
#define _DS18B20_MAX_SENSORS		    DS_SENSORS_NUM
...
uint8_t		DS18B20_GetTemperatureByNumber(uint8_t number, float* destination); // Returns not 0 if read data is invalid
uint8_t		DS18B20_GetTemperatureById(DsNamesId id, float* destination); // Returns not 0 if read data is invalid
